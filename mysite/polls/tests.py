...
class  QuestionViewTests(TestCase):
    ...
    def test_index_view_with_future_and_past_question(self):
        """
        If both past and future questions exist, only show past
        """
        create_question(question_text="Future question.", days=30)
        create_question(question_text="Past question.", days=-30)
        response = self.client.get(reverse('polls:index'))
    ...
    def test_index_view_with_two_past_question(self):
        ...
        """
        create_question(question_text="Past 1 question.", days=-30)
        create_question(question_text="Past 2 question.", days=-5)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
                response.context['latest_question_list'],
                ['<Question: Past 2 question.>', '<Question: Past 1 question.>']
        )
